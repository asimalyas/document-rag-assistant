...
def load_single_file(file_path: str) -> List[Any]:
    """
    Load a single file by its extension and return LangChain documents.
    ...
    loaders = {
        ".pdf": lambda fp: PyPDFLoader(str(fp)),
        ".txt": lambda fp: TextLoader(str(fp)),
        ".csv": lambda fp: CSVLoader(str(fp)),
        ".xlsx": lambda fp: UnstructuredExcelLoader(str(fp)),
        ".docx": lambda fp: Docx2txtLoader(str(fp)),
        ".json": lambda fp: JSONLoader(str(fp)),
    }
    ...
    try:
        ...
        loaded = loader.load()
        # Add source_file and file_type metadata
        for doc in loaded:
            doc.metadata["source_file"] = file_path.name
            doc.metadata["file_type"] = suffix.lstrip(".")
        documents.extend(loaded)
        print(f"[DEBUG] Loaded {len(loaded)} docs from {file_path.name}")
    except Exception as e:
